...
Behavior lives with the data it belongs to:

- bond pricing logic is inside `Bond`,
- event direction logic is inside `Event`,
- processing logic is inside `BondAggregationEngine`.

### Single Responsibility Principle

Each class has one main reason to change:

- data model changes affect `Bond` or `Event`,
- Excel input changes affect `ExcelRepository`,
- reporting logic changes affect `BondAggregationEngine`,
- console formatting changes affect `ReportRenderer`,
- menu flow changes affect `ConsoleMenu`.

### Composition

The system is built by combining objects with focused roles:

- the app uses an engine and renderer,
- the engine uses bonds and events,
- the menu uses the engine and renderer.
...
This makes the program easier to test, maintain, and extend.

## Why This Refactor Is Better

- The script is easier to read because responsibilities are not mixed together.
- The bond and event concepts now exist as real objects instead of anonymous DataFrame rows.
- Future changes like new report types or new data sources have a clear place to live.
- The Excel file paths are now resolved relative to `bofa.py`, so the script works no matter where you launch it from.
